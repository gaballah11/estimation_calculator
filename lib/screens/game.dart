...
final biddingProvider = StateProvider<bool>(
        (ref) => false
);
...
class _gameScState extends ConsumerState<gameSc> {
  ...
  Widget build(BuildContext context) {
    ...
    Gargs = args;
    players = args.map((e) => new inGamePlayer(e)).toList() as List<inGamePlayer>;
                ...
                child: Row(
                  ...
                  children: [
                    ElevatedButton(
                        ...
                        style: ButtonStyle(
                            backgroundColor: indx==rounds.length-1?MaterialStateProperty.all(Colors.redAccent.withOpacity(0.5)):MaterialStateProperty.all(Colors.black87.withOpacity(0.5)),
                            minimumSize: MaterialStateProperty.all(Size.zero),
                            shape: MaterialStateProperty.all(CircleBorder()),
                        ),
                        child:Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text((indx+1).toString(),
                                style: TextStyle(
                                  fontFamily: "Lucida",
                                  fontSize: 12,
                                  fontWeight: FontWeight.w600,
...
class _alert2State extends ConsumerState<alert2> {
                  ...
                  child: Column(
                    ...
                    children: [
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: TextFormField(
                                          ...
                                          validator: (s){
                                            if(int.parse(s!)>13) return "Can't be more than 13";
                                          },
                                          ...
                                          decoration: InputDecoration(
                                            ...
                                            labelText: "Number of tricks Collected",
                      ...
                      ElevatedButton(
                        ...
                        style: ButtonStyle(
                            backgroundColor: MaterialStateProperty.all(Color.fromRGBO(200, 0, 3, 1.0)),
  ...
  List<inGamePlayer> updateScores(List<TextEditingController> calls,List<inGamePlayer> rnd) {
    final scoring score= new scoring();
    List<int> winStatus =[0,0,0,0];
    for(int i=0;i<rnd.length;++i){
      if(rnd[i].bid==int.parse(calls[i].text)){ //win
        winStatus[i]=1;
      }
      print(rnd[i].score);
    }
    ...
    for(int i=0;i<winStatus.length;++i){
      if(winStatus[i] ==1){ //win
        ...
        if(rnd[i].bid ==0 && rnd[i].isCall){ //DashCall
          if(ref.watch(playersProvider.notifier).getSumBids()<13) {
        ...
        else if(rnd[i].bid >= 8 && rnd[i].isCall){ //Big Calls
          ...
          if(rnd[i].isCall) rnd[i].score+=10; // call
          if(rnd[i].isWith) rnd[i].score+=10; // with
          if(wins==1) rnd[i].score += score.onlyWin; //only win
          if(rnd[i].risk!="No Risk"){                  //risk
        ...
        if(rnd[i].bid ==0 && rnd[i].isCall){ //DashCall
          ...
          int x = pow(rnd[i].bid, 2).round();
          rnd[i].score -= (x/2).round();
          continue;
        }
        else{
          rnd[i].score -= score.lose - int.parse(calls[i].text); // regular

          if(rnd[i].isCall) rnd[i].score-=10; // call
          if(rnd[i].isWith) rnd[i].score-=10; // with
          if(wins==3) rnd[i].score -= score.onlyLose; //only lose
          if(rnd[i].risk!="No Risk"){                  //risk
            if(rnd[i].risk=="Risk") rnd[i].score -= 10;
            if(rnd[i].risk=="2x Risk") rnd[i].score -= 20;
            if(rnd[i].risk=="3x Risk") rnd[i].score -= 30;
...
class alertState extends ConsumerState<alert> with AutomaticKeepAliveClientMixin {
    ...
    AlertDialog alert = AlertDialog(
        ...
        content: Consumer(
          builder: (ctx,ref,ch){
            ...
            return StatefulBuilder(
              builder: (BuildContext context, StateSetter setState){
                var colors = ['No Trump','spades','hearts','diamonds','clubs'];
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: TextFormField(
                                          onChanged : (res){
                                            ref.read(playersProvider.notifier).setBid(indx, int.parse(res));
                                          },
                                          style: TextStyle(
                                            fontFamily: "Lucida",
                                            fontStyle: FontStyle.italic,
                                      ...
                                      Container(
                                        ...
                                        child: SwitchListTile.adaptive(
                                            ...
                                            value: ref.watch(playersProvider.notifier).state[indx].isCall,
                                            //autofocus: true ,
                                            onChanged: (val) {
                                      ...
                                      players.state[indx].isCall ? DropdownButton(
                                          ...
                                          isDense: true ,
                                          borderRadius: BorderRadius.circular(40),
                                          hint: Opacity(opacity: 0.5,
                                            child: Image.asset("assets/spades.png", width: sz.width*0.08,),
                                          ),
                                          value: players.state[indx].orderedColor,
                                          items: colors.map((e) {
                                            return DropdownMenuItem(
                                              child: Row(
                                                mainAxisSize: MainAxisSize.min,
                                                children: [
                                                  Image.asset("assets/$e.png",width : sz.width*0.08,),
                                                  Text(e),
                                                ],
                                              ),
                                              value: e,
                                          ...
                                          onChanged: (newVal){
                                            ref.read(playersProvider.notifier).setOColor(indx, newVal.toString());
                                      ...
                                      Container(
                                        ...
                                        child: SwitchListTile.adaptive(
                                            ...
                                            onChanged: (val) {
                                              FocusScope.of(context).unfocus();
                                              ref.read(playersProvider.notifier).toggleIsWith(indx);
                                            }),
                                      ),
                                      DropdownButton(
                                        borderRadius: BorderRadius.circular(40.0),
                                        ...
                                        onChanged: (newVal){
                                          ...
                                          ref.read(playersProvider.notifier).setRisk(indx, newVal.toString());
                                        },
                                      )
                                    ],
                                  ),
